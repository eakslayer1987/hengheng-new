...
function DashboardTab(){
  ...
  if(loading)return<div style={{display:"flex",justifyContent:"center",padding:80}}><Loader2 size={28} style={{animation:"spin 1s linear infinite",color:C.muted}}/></div>;
  ...
  const quotaPct = stats.totalQuotaGiven>0?Math.round((stats.totalQuotaUsed/stats.totalQuotaGiven)*100):0;
  const maxDaily = Math.max(...(stats.dailyData||[]).map(d=>d.count),1);

  const KPI=[
    {label:"โค้ดสะสมทั้งหมด", val:(stats.totalCodes||0).toLocaleString(), sub:`+${stats.todayCollections||0} วันนี้`, icon:"🎟️", grad:"linear-gradient(135deg,#6366F1,#8B5CF6)", light:"#EEF2FF"},
    {label:"ผู้โชคดี",          val:(stats.totalWinners||0).toLocaleString(), sub:`รอรับรางวัล ${stats.pendingClaim||0} คน`, icon:"🏆", grad:"linear-gradient(135deg,#10B981,#059669)", light:"#F0FDF4"},
    {label:"ร้านค้าที่อนุมัติ",  val:(stats.approvedMerchants||0).toLocaleString(), sub:`รอ ${stats.pendingMerchants||0} ร้าน`, icon:"🏪", grad:"linear-gradient(135deg,#F59E0B,#D97706)", light:"#FFFBEB"},
    {label:"ใบเสร็จรอตรวจ",   val:(stats.pendingReceipts||0).toLocaleString(), sub:"รอการอนุมัติ", icon:"📄", grad:"linear-gradient(135deg,#EF4444,#DC2626)", light:"#FFF5F5"},
  ];

  return(
    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} style={{display:"flex",flexDirection:"column",gap:20}}>
      <PageHeader title="Dashboard" sub={stats.campaign?`แคมเปญ: ${stats.campaign.name} — หมดเขต ${new Date(stats.campaign.endDate).toLocaleDateString("th-TH",{day:"numeric",month:"short",year:"numeric"})}`:"ยังไม่มีแคมเปญ"}/>

      {/* Campaign banner */}
      {stats.campaign&&(
        <div style={{background:stats.campaign.isActive?"linear-gradient(135deg,#F0FDF4,#DCFCE7)":"linear-gradient(135deg,#F8FAFC,#F1F5F9)",border:`1px solid ${stats.campaign.isActive?C.greenBorder:C.border}`,borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:10,height:10,borderRadius:"50%",background:stats.campaign.isActive?C.green:C.muted,flexShrink:0,boxShadow:stats.campaign.isActive?"0 0 0 3px rgba(22,163,74,0.2)":"none"}}/>
          <p style={{fontSize:13,fontWeight:600,color:stats.campaign.isActive?C.green:C.sub,margin:0}}>
            {stats.campaign.isActive?"🟢 แคมเปญกำลังดำเนินอยู่":"⏸ แคมเปญปิดอยู่"}
          </p>
          <div style={{marginLeft:"auto",display:"flex",gap:16}}>
            <div style={{textAlign:"right"}}>
              <p style={{fontSize:10,color:C.muted,margin:0}}>อัตราชนะ</p>
              <p style={{fontSize:14,fontWeight:700,color:C.text,margin:0}}>{winRate}%</p>
            </div>
            <div style={{textAlign:"right"}}>
              <p style={{fontSize:10,color:C.muted,margin:0}}>QR ที่ใช้</p>
              <p style={{fontSize:14,fontWeight:700,color:C.text,margin:0}}>{quotaPct}%</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
        {KPI.map((k,i)=>(
          <div key={i} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",overflow:"hidden",position:"relative"}}>
            <div style={{position:"absolute",top:-10,right:-10,width:60,height:60,borderRadius:"50%",background:k.grad,opacity:0.08}}/>
            <div style={{width:38,height:38,borderRadius:10,background:k.light,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,marginBottom:12}}>{k.icon}</div>
            <p style={{fontSize:22,fontWeight:700,color:C.text,margin:"0 0 2px"}}>{k.val}</p>
            <p style={{fontSize:11,fontWeight:500,color:C.muted,margin:"0 0 4px"}}>{k.label}</p>
            <p style={{fontSize:11,color:C.muted,margin:0,opacity:.7}}>{k.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:14}}>
        ...
        <Card>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
            <p style={{fontSize:14,fontWeight:600,color:C.text,margin:0}}>📊 สแกน 7 วันล่าสุด</p>
            <span style={{fontSize:11,color:C.muted}}>รายวัน</span>
          </div>
          {stats.dailyData.length>0
            ? <div style={{display:"flex",alignItems:"flex-end",gap:8,height:120}}>
                {stats.dailyData.map((d,i)=>(
                  <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                    <p style={{fontSize:11,fontWeight:600,color:d.count>0?C.blue:C.muted,margin:0}}>{d.count||""}</p>
                    <div style={{width:"100%",borderRadius:"4px 4px 0 0",minHeight:4,
                      background:d.count>0?`linear-gradient(180deg,#6366F1,#4F46E5)`:"#F1F5F9",
                      height:`${Math.max((d.count/maxDaily)*90,4)}px`,transition:"height .4s ease"}}/>
            ...
            : <p style={{color:C.muted,fontSize:13,textAlign:"center",padding:30}}>ยังไม่มีข้อมูล</p>
          }
        </Card>

        {/* Prizes */}
        <Card>
          <p style={{fontSize:14,fontWeight:600,color:C.text,margin:"0 0 14px"}}>🏅 รางวัลคงเหลือ</p>
          {stats.prizes.length>0
            ? <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {stats.prizes.map((p,i)=>{
                  const pct=Math.round((p.remaining/p.quantity)*100);
                  const barColor=pct>50?"#6366F1":pct>20?C.gold:C.red;
                  return(
                    <div key={p.id}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                        ...
                        <span style={{fontSize:12,fontWeight:600,color:barColor}}>{p.remaining}<span style={{color:C.muted,fontWeight:400}}>/{p.quantity}</span></span>
                      </div>
            ...
            : <p style={{color:C.muted,fontSize:13,textAlign:"center",padding:20}}>ยังไม่มีรางวัล</p>
          }
        </Card>
      </div>

      {/* Quota progress */}
      {stats.totalQuotaGiven>0&&(
        <Card>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
            <p style={{fontSize:14,fontWeight:600,color:C.text,margin:0}}>📦 QR Quota Overview</p>
            <span style={{fontSize:12,color:C.muted}}>{stats.totalQuotaUsed.toLocaleString()} / {stats.totalQuotaGiven.toLocaleString()}</span>
          </div>
          <div style={{height:8,borderRadius:4,background:C.bg,overflow:"hidden",marginBottom:6}}>
            <div style={{height:"100%",borderRadius:4,background:"linear-gradient(90deg,#6366F1,#8B5CF6)",width:`${quotaPct}%`,transition:"width .8s"}}/>
          </div>
          <div style={{display:"flex",justifyContent:"space-between"}}>
            <span style={{fontSize:11,color:C.muted}}>ใช้ไป {quotaPct}%</span>
            <span style={{fontSize:11,color:C.green,fontWeight:600}}>คงเหลือ {stats.totalQuotaRemaining.toLocaleString()} โค้ด</span>
          </div>
        </Card>
      )}
    </motion.div>
              ...
              return(
                <Card key={rc.id} style={{border:rc.status==="pending"?`1px solid ${C.goldBorder}`:`1px solid ${C.border}`}}>
                  <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                    {/* Thumbnail */}
                    <img src={rc.imageUrl} onClick={()=>setPreviewImg(rc.imageUrl)} style={{width:64,height:64,borderRadius:8,objectFit:"cover",cursor:"pointer",border:`1px solid ${C.border}`,flexShrink:0}}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
...
function SettingsTab({toast}:{toast:(m:string,ok?:boolean)=>void}){
  ...
  const [subTab,setSubTab]=useState<"campaign"|"prizes"|"system"|"otp"|"account">("campaign");
  ...
  const saveCampaign=async()=>{
    ...
    const payload={id:editCamp.id,name:editCamp.name,description:editCamp.description||"",startDate:editCamp.startDate?.slice(0,10),endDate:editCamp.endDate?.slice(0,10),totalBudget:Number(editCamp.totalBudget)||1000000,codesPerBag:Number(editCamp.codesPerBag)||30};
  ...
  if(loading)return<div style={{display:"flex",justifyContent:"center",padding:80}}><Loader2 size={24} style={{animation:"spin 1s linear infinite",color:C.muted}}/></div>;
  const SUB_TABS=[{key:"campaign",label:"🗓 แคมเปญ"},{key:"prizes",label:"🏆 รางวัล"},{key:"system",label:"⚙️ ระบบ"},{key:"otp",label:"🔐 OTP"},{key:"account",label:"👤 บัญชี"}] as const;
  const filteredPrizes=prizes.filter(p=>!editCamp||p.campaignId===editCamp.id);
  const totalBudget=filteredPrizes.reduce((s,p)=>s+p.value*p.quantity,0);
  return(
    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
      <PageHeader title="ตั้งค่าระบบ" sub="จัดการแคมเปญ รางวัล และการตั้งค่าทั้งหมด"/>
      <div style={{display:"flex",gap:4,marginBottom:20,background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:4}}>
        {SUB_TABS.map(t=>(
          <button key={t.key} onClick={()=>setSubTab(t.key)}
            style={{flex:1,padding:"8px 4px",borderRadius:8,border:"none",cursor:"pointer",fontSize:12,fontWeight:subTab===t.key?700:500,
              background:subTab===t.key?"#6366F1":"transparent",color:subTab===t.key?"#fff":C.sub,transition:"all .15s"}}>
            {t.label}
          </button>
        ))}
      </div>
      {subTab==="campaign"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {campaigns.map(c=>{
            const isSelected=editCamp?.id===c.id;
            const daysLeft=Math.ceil((new Date(c.endDate).getTime()-Date.now())/(86400000));
            return(
              <div key={c.id} onClick={()=>setEditCamp({...c})}
                style={{background:C.card,border:`2px solid ${isSelected?"#6366F1":C.border}`,borderRadius:14,padding:"16px 18px",cursor:"pointer",transition:"all .15s",boxShadow:isSelected?"0 0 0 4px rgba(99,102,241,0.1)":"none"}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <div style={{width:44,height:44,borderRadius:12,background:c.isActive?"linear-gradient(135deg,#6366F1,#8B5CF6)":"#F1F5F9",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{c.isActive?"🟢":"⏸"}</div>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                      <p style={{fontWeight:700,fontSize:14,color:C.text,margin:0}}>{c.name}</p>
                      <Badge color={c.isActive?C.green:C.muted}>{c.isActive?"กำลังดำเนินการ":"ปิดอยู่"}</Badge>
                      {isSelected&&<Badge color="#6366F1">✏️ กำลังแก้ไข</Badge>}
                    </div>
                    <p style={{fontSize:12,color:C.muted,margin:0}}>{new Date(c.startDate).toLocaleDateString("th-TH")} → {new Date(c.endDate).toLocaleDateString("th-TH")} · {(c._count?.codes||0).toLocaleString()} โค้ด{daysLeft>0?` · เหลือ ${daysLeft} วัน`:" · หมดเขตแล้ว"}</p>
                  </div>
                  <button onClick={e=>{e.stopPropagation();toggleActive(c.id,!c.isActive);}} style={{background:"none",border:"none",cursor:"pointer",padding:0,flexShrink:0}}>
                    {c.isActive?<ToggleRight size={28} style={{color:"#6366F1"}}/>:<ToggleLeft size={28} style={{color:C.muted}}/>}
                  </button>
                </div>
              </div>
            );
          ...
          {editCamp&&(
            <Card style={{border:"1px solid rgba(99,102,241,0.3)",background:"rgba(99,102,241,0.03)"}}>
              <p style={{fontSize:13,fontWeight:700,color:"#6366F1",margin:"0 0 14px"}}>✏️ แก้ไข: {editCamp.name}</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                <div style={{gridColumn:"1/-1"}}><InputField label="ชื่อแคมเปญ" value={editCamp.name||""} onChange={v=>setEditCamp(p=>p?{...p,name:v}:p)}/></div>
                <InputField label="วันเริ่ม" type="date" value={editCamp.startDate?.slice(0,10)||""} onChange={v=>setEditCamp(p=>p?{...p,startDate:v}:p)}/>
                <InputField label="วันหมดเขต" type="date" value={editCamp.endDate?.slice(0,10)||""} onChange={v=>setEditCamp(p=>p?{...p,endDate:v}:p)}/>
                <InputField label="งบประมาณ (บาท)" type="number" value={editCamp.totalBudget||0} onChange={v=>setEditCamp(p=>p?{...p,totalBudget:Number(v)}:p)}/>
                <InputField label="QR ต่อถุง" type="number" value={editCamp.codesPerBag||30} onChange={v=>setEditCamp(p=>p?{...p,codesPerBag:Number(v)}:p)} note="ค่าเริ่มต้น 30"/>
              </div>
          ...
          {!newCamp
            ...
            :<Card style={{border:`1px solid ${C.blueBorder}`,background:C.blueLight}}>
              <p style={{fontSize:13,fontWeight:700,color:C.blue,margin:"0 0 14px"}}>+ สร้างแคมเปญใหม่</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
      ...
      {subTab==="prizes"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {filteredPrizes.length>0&&(
            <div style={{background:"linear-gradient(135deg,#FFFBEB,#FEF3C7)",border:`1px solid ${C.goldBorder}`,borderRadius:12,padding:"14px 18px",display:"flex",gap:24}}>
              <div><p style={{fontSize:11,color:C.gold,margin:0}}>รางวัลทั้งหมด</p><p style={{fontSize:18,fontWeight:700,color:C.text,margin:0}}>{filteredPrizes.length} รายการ</p></div>
              <div><p style={{fontSize:11,color:C.gold,margin:0}}>มูลค่ารวม</p><p style={{fontSize:18,fontWeight:700,color:C.text,margin:0}}>฿{totalBudget.toLocaleString()}</p></div>
              <div><p style={{fontSize:11,color:C.gold,margin:0}}>คงเหลือรวม</p><p style={{fontSize:18,fontWeight:700,color:C.green,margin:0}}>{filteredPrizes.reduce((s,p)=>s+p.remaining,0)} ชิ้น</p></div>
            </div>
          )}
            ...
            {filteredPrizes.map((p,i)=>{
              ...
              return(
                <Card key={p.id} style={{position:"relative",overflow:"hidden"}}>
                  <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:barColor,opacity:.7}}/>
                  <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:12}}>
                    <span style={{fontSize:28}}>{["🥇","🥈","🥉","🎁","🎀","🎖️"][i]||"🎁"}</span>
                    <div style={{flex:1}}><p style={{fontSize:14,fontWeight:700,color:C.text,margin:0}}>{p.name}</p><p style={{fontSize:12,color:C.muted,margin:"2px 0 0"}}>฿{p.value.toLocaleString()} / ชิ้น</p></div>
                    <button onClick={()=>deletePrize(p.id)} style={{background:"#FEF2F2",border:`1px solid ${C.redBorder}`,borderRadius:6,padding:"3px 6px",cursor:"pointer",color:C.red}}><Trash2 size={12}/></button>
                  </div>
                  <div style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      <span style={{fontSize:11,color:C.muted}}>คงเหลือ</span>
                      <span style={{fontSize:12,fontWeight:700,color:barColor}}>{p.remaining}/{p.quantity}</span>
                    </div>
                    <div style={{height:6,borderRadius:3,background:C.bg}}><div style={{height:"100%",borderRadius:3,background:barColor,width:`${pct}%`,transition:"width .4s"}}/></div>
                  </div>
                  <p style={{fontSize:11,color:C.muted,margin:0}}>มูลค่ารวม ฿{(p.value*p.quantity).toLocaleString()}</p>
                </Card>
              );
            })}
          </div>
          {!showNewPrize
            ...
            :<Card style={{border:`1px solid ${C.greenBorder}`,background:C.greenLight}}>
              <p style={{fontSize:13,fontWeight:700,color:C.green,margin:"0 0 12px"}}>+ เพิ่มรางวัลใหม่</p>
              <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:10,marginBottom:12}}>
                <InputField label="ชื่อรางวัล *" value={newPrize.name} onChange={v=>setNewPrize(p=>({...p,name:v}))} placeholder="เช่น เงินสด 100,000 บาท"/>
                <InputField label="มูลค่า (฿)" type="number" value={newPrize.value} onChange={v=>setNewPrize(p=>({...p,value:v}))}/>
                <InputField label="จำนวน" type="number" value={newPrize.quantity} onChange={v=>setNewPrize(p=>({...p,quantity:v}))}/>
              </div>
              {newPrize.value&&newPrize.quantity&&(<p style={{fontSize:12,color:C.green,marginBottom:10}}>มูลค่ารวม: ฿{(Number(newPrize.value)*Number(newPrize.quantity)).toLocaleString()}</p>)}
              <div style={{display:"flex",gap:8}}>
      ...
      {subTab==="system"&&(
        <Card>
          <p style={{fontSize:14,fontWeight:600,color:C.text,margin:"0 0 16px"}}>⚙️ ตัวแปรระบบสะสมโค้ด</p>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:14,marginBottom:16}}>
            <InputField label="🗓 สิทธิ์สะสมสูงสุด/วัน/คน" type="number" value={cf("daily_collect_limit")||"3"} onChange={v=>setCf("daily_collect_limit",v)} note="นับรวมทุกร้าน"/>
          ...
          <div style={{background:"#F8FAFC",border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px",marginBottom:14,display:"flex",gap:20}}>
            <div style={{textAlign:"center"}}><p style={{fontSize:22,fontWeight:800,color:"#6366F1",margin:0}}>{cf("daily_collect_limit")||"3"}</p><p style={{fontSize:11,color:C.muted,margin:0}}>ครั้ง/วัน</p></div>
            <div style={{textAlign:"center"}}><p style={{fontSize:22,fontWeight:800,color:C.gold,margin:0}}>{cf("gps_radius_m")||"20"}m</p><p style={{fontSize:11,color:C.muted,margin:0}}>รัศมี GPS</p></div>
          </div>
      ...
      {subTab==="otp"&&(
        <Card>
          <p style={{fontSize:14,fontWeight:600,color:C.text,margin:"0 0 4px"}}>🔐 ตั้งค่า OTP</p>
          <p style={{fontSize:12,color:C.muted,margin:"0 0 16px"}}>กำหนดอายุและช่วงเวลา cooldown ของ OTP</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            <div>
              ...
              <div style={{background:"#EFF6FF",borderRadius:8,padding:"8px 12px",marginTop:8}}><p style={{fontSize:12,color:C.blue,margin:0}}>⏱ OTP หมดอายุใน {cf("otp_expire_min")||"5"} นาที</p></div>
            </div>
            <div>
              ...
              <div style={{background:"#F0FDF4",borderRadius:8,padding:"8px 12px",marginTop:8}}><p style={{fontSize:12,color:C.green,margin:0}}>🔄 Cooldown {cf("otp_cooldown_sec")||"60"} วินาที</p></div>
            </div>
          </div>
      ...
      {subTab==="account"&&(
        <Card>
          <div style={{display:"flex",gap:14,alignItems:"center",marginBottom:20,padding:"14px 16px",background:"#F8FAFC",borderRadius:10,border:`1px solid ${C.border}`}}>
            <div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#6366F1,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,color:"#fff",fontWeight:800}}>A</div>
            <div><p style={{fontSize:15,fontWeight:700,color:C.text,margin:0}}>Administrator</p><p style={{fontSize:12,color:C.muted,margin:0}}>บัญชีระดับสูงสุด — มีสิทธิ์เต็ม</p></div>
          </div>
          <p style={{fontSize:13,fontWeight:600,color:C.text,margin:"0 0 12px"}}>เปลี่ยนรหัสผ่าน</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          ...
          {newAdminPw.length>0&&newAdminPw.length<6&&(<p style={{fontSize:11,color:C.red,marginBottom:10}}>⚠️ รหัสผ่านต้องยาวอย่างน้อย 6 ตัวอักษร</p>)}
          <Btn variant="danger" onClick={changeAdminPassword} disabled={newAdminPw.length>0&&newAdminPw.length<6}><Shield size={12}/> เปลี่ยนรหัสผ่าน</Btn>
        </Card>
      )}
    </motion.div>
  );
...
function ContentTab({toast}:{toast:(m:string,ok?:boolean)=>void}){
  ...
  const [subTab,setSubTab]=useState<"app"|"nav"|"steps"|"slides">("app");

  const load=async()=>{
    setLoading(true);
    const r=await fetch("/api/admin/config"); const d=await r.json();
    if(d.config){
      setConfig(d.config); setLocal(d.config);
      try{
        const raw=JSON.parse(d.config.how_to_steps||"[]");
        // normalize: รองรับ format เก่า {emoji,title,sub} และใหม่ {n,e,t,d}
        setSteps(Array.isArray(raw)?raw.filter(Boolean).map((s:any,i:number)=>({
          n: s.n ?? i+1,
          e: s.e ?? s.emoji ?? "✨",
          t: s.t ?? s.title ?? "",
          d: s.d ?? s.sub ?? "",
      ...
      try{
        const raw=JSON.parse(d.config.hero_slides||"[]");
        setSlides(Array.isArray(raw)?raw.filter(Boolean).map((s:any)=>({
          emoji: s.emoji ?? s.e ?? "✨",
          title: s.title ?? s.t ?? "",
          sub:   s.sub   ?? s.d ?? "",
        })):[]);
  ...
  const updateStep=(i:number,field:string,val:string)=>{const ns=[...steps];(ns[i] as any)[field]=val;setSteps(ns);};
  const saveSteps=()=>save(["how_to_steps"],{how_to_steps:JSON.stringify(steps)});
  const updateSlide=(i:number,field:string,val:string)=>{const ns=[...slides];(ns[i] as any)[field]=val;setSlides(ns);};
  const saveSlides=()=>save(["hero_slides"],{hero_slides:JSON.stringify(slides)});
  if(loading)return<div style={{display:"flex",justifyContent:"center",padding:80}}><Loader2 size={24} style={{animation:"spin 1s linear infinite",color:C.muted}}/></div>;

  const CTABS=[{key:"app",label:"📱 แอป"},{key:"nav",label:"🔘 เมนู"},{key:"steps",label:"📋 ขั้นตอน"},{key:"slides",label:"🖼 Slides"}] as const;
  ...
  return(
    <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
      ...
      {subTab==="app"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <Card>
            <p style={{fontSize:14,fontWeight:600,color:C.text,margin:"0 0 14px"}}>📱 ส่วนหัวแอป</p>
            ...
            {/* Live preview */}
            <div style={{background:"linear-gradient(135deg,#1a0000,#2a0505)",borderRadius:12,padding:"16px",marginBottom:14,textAlign:"center"}}>
              <p style={{fontSize:16,fontWeight:800,color:"#FFD700",margin:"0 0 4px"}}>{cf("app_name")||"ชื่อแอป"}</p>
              <p style={{fontSize:12,color:"rgba(255,255,255,0.6)",margin:0}}>{cf("app_subtitle")||"คำโปรย"}</p>
            </div>
            <Btn onClick={()=>save(["app_name","app_subtitle"])} disabled={saving==="app_name"}>{saving==="app_name"?<Loader2 size={12} style={{animation:"spin 1s linear infinite"}}/>:<Save size={12}/>} บันทึก</Btn>
          </Card>
          <Card>
            <p style={{fontSize:14,fontWeight:600,color:C.text,margin:"0 0 14px"}}>🎯 ปุ่มสแกน & หัวข้อ</p>
            ...
            <Btn onClick={()=>save(["scan_btn_text","prize_section_title","daily_limit_title","daily_limit_sub"])} disabled={saving==="scan_btn_text"}>{saving==="scan_btn_text"?<Loader2 size={12} style={{animation:"spin 1s linear infinite"}}/>:<Save size={12}/>} บันทึก</Btn>
          </Card>
        </div>
      )}
      ...
      {subTab==="nav"&&(
        <Card>
          <p style={{fontSize:14,fontWeight:600,color:C.text,margin:"0 0 6px"}}>🔘 เมนูด้านล่าง (Bottom Nav)</p>
          <p style={{fontSize:12,color:C.muted,margin:"0 0 14px"}}>ข้อความที่แสดงใต้ icon แต่ละปุ่ม</p>
          {/* Preview */}
          <div style={{background:"#0F172A",borderRadius:12,padding:"12px 8px",marginBottom:14,display:"flex",justifyContent:"space-around"}}>
            {[{key:"nav_home",icon:"🏠"},{key:"nav_prizes",icon:"🏆"},{key:"nav_scan",icon:"📷"},{key:"nav_check",icon:"🔍"},{key:"nav_more",icon:"•••"}].map(f=>(
              <div key={f.key} style={{textAlign:"center"}}>
                <p style={{fontSize:18,margin:"0 0 2px"}}>{f.icon}</p>
                <p style={{fontSize:9,color:"rgba(255,255,255,0.6)",margin:0,maxWidth:40}}>{cf(f.key)||"..."}</p>
              </div>
            ))}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14}}>
            {[{key:"nav_home",label:"🏠 หน้าหลัก"},{key:"nav_prizes",label:"🏆 รางวัล"},{key:"nav_scan",label:"📷 สแกน (กลาง)"},{key:"nav_check",label:"🔍 ตรวจสอบ"},{key:"nav_more",label:"••• อื่นๆ"}].map(f=>(
              <InputField key={f.key} label={f.label} value={cf(f.key)} onChange={v=>setCf(f.key,v)}/>
            ))}
          </div>
          <Btn onClick={()=>save(["nav_home","nav_prizes","nav_scan","nav_check","nav_more"])} disabled={saving==="nav_home"}>{saving==="nav_home"?<Loader2 size={12} style={{animation:"spin 1s linear infinite"}}/>:<Save size={12}/>} บันทึก</Btn>
        </Card>
      )}
      ...
      {subTab==="steps"&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
            <p style={{fontSize:14,fontWeight:600,color:C.text,margin:0}}>📋 ขั้นตอนวิธีรับโค้ด</p>
            <span style={{fontSize:12,color:C.muted}}>{steps.length} ขั้นตอน</span>
          </div>
          {steps.filter(Boolean).map((s,i)=>(
            <Card key={i} style={{borderLeft:`3px solid #6366F1`}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <div style={{width:26,height:26,borderRadius:"50%",background:"#6366F1",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"#fff",flexShrink:0}}>{s.n}</div>
                <p style={{fontSize:13,fontWeight:600,color:C.text,margin:0}}>ขั้นตอนที่ {s.n}</p>
              </div>
          ...
          <Btn onClick={saveSteps} disabled={saving==="how_to_steps"}>{saving==="how_to_steps"?<Loader2 size={12} style={{animation:"spin 1s linear infinite"}}/>:<Save size={12}/>} บันทึกขั้นตอนทั้งหมด</Btn>
        </div>
      )}
      ...
      {subTab==="slides"&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
            <p style={{fontSize:14,fontWeight:600,color:C.text,margin:0}}>🖼 Hero Slides (Fallback)</p>
            <Badge color={C.muted}>ใช้เมื่อไม่มี Banner Hero</Badge>
          </div>
          {slides.filter(Boolean).map((s,i)=>(
            <Card key={i} style={{borderLeft:`3px solid ${C.gold}`}}>
              <p style={{fontSize:12,fontWeight:600,color:C.gold,margin:"0 0 10px"}}>Slide {i+1}</p>
              {/* Preview */}
              <div style={{background:"linear-gradient(135deg,#1a0000,#2a0505)",borderRadius:8,padding:"12px",marginBottom:10,textAlign:"center"}}>
                <p style={{fontSize:28,margin:"0 0 4px"}}>{s.emoji||"✨"}</p>
                <p style={{fontSize:13,fontWeight:700,color:"#FFD700",margin:"0 0 2px"}}>{s.title||"หัวข้อ"}</p>
                <p style={{fontSize:11,color:"rgba(255,255,255,0.6)",margin:0}}>{s.sub||"คำอธิบาย"}</p>
              </div>
          ...
          <Btn onClick={saveSlides} disabled={saving==="hero_slides"}>{saving==="hero_slides"?<Loader2 size={12} style={{animation:"spin 1s linear infinite"}}/>:<Save size={12}/>} บันทึก Slides ทั้งหมด</Btn>
        </div>
      )}
    </motion.div>
  );
...
const POSITIONS = [
  {value:"user_topbar",    label:"ผู้ใช้ — Top bar"},
  {value:"user_hero",      label:"ผู้ใช้ — Hero"},
  {value:"user_infeed",    label:"ผู้ใช้ — In-feed"},
  {value:"user_popup",     label:"ผู้ใช้ — Popup"},
  {value:"user_sticky",    label:"ผู้ใช้ — Sticky"},
  {value:"merchant_hero",  label:"ร้านค้า — Hero"},
  {value:"merchant_card",  label:"ร้านค้า — Card"},
  {value:"merchant_popup", label:"ร้านค้า — Popup"},
  {value:"admin_alert",    label:"แอดมิน — Alert"},
];
...
const BTYPES = [
  ...
  {value:"sticky",       label:"Sticky Bar (ล่าง)"},
  {value:"popup",        label:"Popup"},
  {value:"interstitial", label:"Interstitial (เต็มจอ)"},
];

const BFORM0 = {
  position:"user_hero", type:"announcement", title:"", body:"",
  imageUrl:"", linkUrl:"", linkTarget:"_blank",
  ctaText:"", bgColor:"#FD1803", textColor:"#FFFFFF",
              ...
              <Card key={b.id} style={{padding:"12px 14px",border:b.isActive?`1px solid ${C.greenBorder}`:`1px solid ${C.border}`}}>
                <div style={{display:"flex",gap:12,alignItems:"center"}}>
                  <div style={{width:36,height:36,borderRadius:8,background:b.bgColor,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,overflow:"hidden"}}>
                    {b.imageUrl?<img src={b.imageUrl} style={{width:36,height:36,objectFit:"cover"}}/>:emoji(b.type)}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
...
/* ════ QR RULES TAB ════ */
type QrRule = {
  id: number; name: string; ruleType: string; value: string;
  blockMessage: string; isActive: boolean; priority: number;
};
const RULE_TYPES = [
  { value: "time",         label: "⏰ ตามเวลา",            hint: "เช่น 08:00-17:00" },
  { value: "day_of_week",  label: "📅 ตามวัน",             hint: "เช่น 1,2,3,4,5 (จ-ศ)" },
  { value: "scan_count",   label: "🔢 ตามจำนวนสแกน",      hint: 'เช่น {"from":1,"to":100}' },
  { value: "geo",          label: "📍 รัศมีสถานที่ (m)",   hint: "เช่น 50" },
];
const RULE_PRESETS: Record<string, { value: string; blockMessage: string }> = {
  time:        { value: "08:00-21:00", blockMessage: "ขออภัย สามารถสแกนได้เฉพาะเวลา 08:00-21:00 น. เท่านั้น" },
  day_of_week: { value: "1,2,3,4,5",  blockMessage: "ขออภัย สามารถสแกนได้เฉพาะวันจันทร์-ศุกร์ เท่านั้น" },
  scan_count:  { value: '{"from":1,"to":1000}', blockMessage: "โควต้าการสแกนเต็มแล้ว กรุณาติดต่อร้านค้า" },
  geo:         { value: "50", blockMessage: "คุณอยู่นอกพื้นที่ร้านค้า กรุณาสแกนภายในร้านเท่านั้น 📍" },
};
...
function QrRulesTab({ toast }: { toast: (m: string, ok?: boolean) => void }) {
  ...
  const blank = { name: "", ruleType: "time", value: "", blockMessage: "", priority: 0 };
  ...
  const load = async () => {
    setLoading(true);
    const r = await fetch("/api/admin/qr-rules");
  ...
  const openNew = () => {
    setEditing(null);
    const preset = RULE_PRESETS["time"];
    setForm({ name: "", ruleType: "time", value: preset.value, blockMessage: preset.blockMessage, priority: 0 });
    setShowForm(true);
  };
  const openEdit = (r: QrRule) => {
    setEditing(r);
    setForm({ name: r.name, ruleType: r.ruleType, value: r.value, blockMessage: r.blockMessage, priority: r.priority });
    setShowForm(true);
  };

  const onTypeChange = (t: string) => {
    const preset = RULE_PRESETS[t] || { value: "", blockMessage: "" };
    setForm(p => ({ ...p, ruleType: t, value: preset.value, blockMessage: preset.blockMessage }));
  };

  const handleSave = async () => {
    if (!form.name.trim() || !form.value.trim() || !form.blockMessage.trim()) {
      toast("กรุณากรอกข้อมูลให้ครบ", false); return;
    }
    setSaving(true);
  ...
  const toggleActive = async (rule: QrRule) => {
    ...
    toast(rule.isActive ? "ปิด rule แล้ว" : "เปิด rule แล้ว");
    load();
  };
  ...
  const ruleTypeHint  = (t: string) => RULE_TYPES.find(r => r.value === t)?.hint || "";
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <Card style={{ marginBottom: 16, background: C.blueLight, border: `1px solid ${C.blueBorder}` }}>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div style={{ fontSize: 13, color: C.blue, lineHeight: 1.6 }}>
            <strong>วิธีทำงาน:</strong> เมื่อลูกค้าสแกน QR ระบบจะ evaluate rules ตาม priority (น้อย = ก่อน)
            — rule ไหนไม่ผ่านจะแสดง blockMessage และหยุดทันที
          </div>
        </div>
      </Card>
      ...
      {loading ? (
        ...
        <Card style={{ textAlign: "center", padding: "48px 24px" }}>
          <ScanLine size={32} style={{ color: C.muted, margin: "0 auto 12px" }} />
          <p style={{ color: C.muted, fontSize: 14 }}>ยังไม่มี Smart QR Rules</p>
          <p style={{ color: C.muted, fontSize: 12, marginTop: 4 }}>กด "เพิ่ม Rule" เพื่อตั้งค่ากฎการสแกน</p>
        </Card>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {rules.map(rule => (
            <Card key={rule.id} style={{ opacity: rule.isActive ? 1 : 0.55 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                {/* Priority badge */}
                <div style={{
                  minWidth: 36, height: 36, borderRadius: 8, background: C.blueLight,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                    ...
                    <Badge color={C.blue}>{ruleTypeLabel(rule.ruleType)}</Badge>
                  </div>
                  <code style={{
                    fontSize: 12, background: C.cardAlt, border: `1px solid ${C.border}`,
                    borderRadius: 6, padding: "2px 8px", color: C.sub, display: "inline-block", marginBottom: 6,
                  }}>
                    {rule.value}
                  </code>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <Btn size="sm" variant="secondary" onClick={() => toggleActive(rule)}>
                    {rule.isActive ? <EyeOff size={13} /> : <Eye size={13} />}
                  </Btn>
                  <Btn size="sm" variant="secondary" onClick={() => openEdit(rule)}>
                    <Pencil size={13} />
                  </Btn>
                  <Btn size="sm" variant="danger" onClick={() => handleDelete(rule.id)}>
                    <Trash2 size={13} />
                  </Btn>
                </div>
              </div>
      ...
      {showForm && (
        <div style={{
          ...
          zIndex: 100, padding: "0 0 0 220px",
        }}>
          <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            style={{
              background: C.card, borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 560,
              maxHeight: "85vh", overflow: "auto", padding: "24px",
              boxShadow: "0 -8px 40px rgba(0,0,0,0.15)",
            }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: C.text, margin: "0 0 20px" }}>
              {editing ? "แก้ไข Rule" : "เพิ่ม Smart QR Rule"}
            </h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <InputField label="ชื่อ Rule" value={form.name} onChange={v => sf("name", v)}
                placeholder="เช่น เปิดเฉพาะวันธรรมดา" required />

              {/* Rule Type */}
              <div>
                <label style={{ fontSize: 12, fontWeight: 500, color: C.sub, display: "block", marginBottom: 5 }}>
                  ประเภท Rule <span style={{ color: C.red }}>*</span>
                </label>
                  ...
                  {RULE_TYPES.map(rt => (
                    <button key={rt.value} onClick={() => onTypeChange(rt.value)}
                      style={{
                        ...
                        border: `1px solid ${form.ruleType === rt.value ? C.blue : C.border}`,
                        background: form.ruleType === rt.value ? C.blueLight : C.card,
                        color: form.ruleType === rt.value ? C.blue : C.sub,
                        fontWeight: form.ruleType === rt.value ? 600 : 400,
                      }}>
                      {rt.label}
                      <span style={{ display: "block", fontSize: 10, color: C.muted, marginTop: 2 }}>{rt.hint}</span>
                    </button>
                  ))}
                </div>
              </div>

              <InputField label={`ค่า (${ruleTypeHint(form.ruleType)})`} value={form.value}
                onChange={v => sf("value", v)} placeholder={ruleTypeHint(form.ruleType)} required />

              {/* Block message */}
              <div>
                <label style={{ fontSize: 12, fontWeight: 500, color: C.sub, display: "block", marginBottom: 5 }}>
                  ข้อความแสดงเมื่อถูก block <span style={{ color: C.red }}>*</span>
                </label>
                <textarea value={form.blockMessage} onChange={e => sf("blockMessage", e.target.value)} rows={3}
                  placeholder="เช่น ขออภัย สามารถสแกนได้เฉพาะ..."
                  style={{
                    width: "100%", boxSizing: "border-box", padding: "8px 10px",
                    border: `1px solid ${C.borderMid}`, borderRadius: 8, fontSize: 13,
                    color: C.text, outline: "none", resize: "none", fontFamily: "inherit",
                  }} />
                {/* Preview */}
                {form.blockMessage && (
                  <div style={{
                    marginTop: 6, background: "#FEF2F2", border: `1px solid ${C.redBorder}`,
                    borderRadius: 8, padding: "8px 12px", fontSize: 12, color: C.red,
                  }}>
                    🚫 Preview: {form.blockMessage}
                  </div>
                )}
              </div>

              <InputField label="Priority (น้อย = ตรวจก่อน)" type="number"
                value={form.priority} onChange={v => sf("priority", Number(v))} note="0-99 แนะนำ" />

              <div style={{ display: "flex", gap: 8, paddingTop: 8, borderTop: `1px solid ${C.border}` }}>
                <Btn onClick={handleSave} disabled={saving}>
                  {saving ? <Loader2 size={13} style={{ animation: "spin 1s linear infinite" }} /> : <Save size={13} />}
                  {editing ? "บันทึก" : "สร้าง Rule"}
                </Btn>
                <Btn variant="ghost" onClick={() => setShowForm(false)}>ยกเลิก</Btn>
              </div>
            </div>
...
export default function AdminClient(){
  ...
  const NAV_GROUPS=[
    {label:"ภาพรวม",items:[{key:"dashboard",icon:<BarChart3 size={15}/>,label:"Dashboard",badge:0}]},
    {label:"แคมเปญ",items:[{key:"entries",icon:<Users size={15}/>,label:"ผู้เข้าร่วม",badge:0},{key:"codes",icon:<QrCode size={15}/>,label:"Lucky Code",badge:0},{key:"luckydraw",icon:<Trophy size={15}/>,label:"จับรางวัล",badge:0}]},
    {label:"ร้านค้า",items:[{key:"merchants",icon:<Store size={15}/>,label:"ร้านค้า",badge:pendingMerchantsCount},{key:"receipts",icon:<FileText size={15}/>,label:"ใบเสร็จ",badge:pendingCount}]},
    {label:"ระบบ",items:[{key:"settings",icon:<Settings size={15}/>,label:"ตั้งค่า",badge:0},{key:"content",icon:<Bell size={15}/>,label:"เนื้อหา",badge:0},{key:"banners",icon:<Tag size={15}/>,label:"แบนเนอร์",badge:0},{key:"qrrules",icon:<ScanLine size={15}/>,label:"Smart QR",badge:0}]},
  ];

  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}.sb-item:hover{background:rgba(255,255,255,0.05)!important}`}</style>
      <div style={{position:"fixed",top:16,right:16,zIndex:200,display:"flex",flexDirection:"column",gap:8,pointerEvents:"none"}}>
        <AnimatePresence>{toasts.map(t=>(<motion.div key={t.id} initial={{opacity:0,x:40}} animate={{opacity:1,x:0}} exit={{opacity:0,x:40}} style={{background:t.ok?C.green:C.red,color:"#fff",padding:"10px 16px",borderRadius:10,fontSize:13,fontWeight:500,pointerEvents:"auto",boxShadow:"0 4px 16px rgba(0,0,0,0.15)"}}>{t.ok?"✅":"❌"} {t.msg}</motion.div>))}</AnimatePresence>
      </div>
      <aside style={{width:232,minHeight:"100vh",background:C.sidebar,display:"flex",flexDirection:"column",position:"fixed",left:0,top:0,bottom:0,zIndex:10,boxShadow:"4px 0 24px rgba(0,0,0,0.3)"}}>
        <div style={{padding:"20px 16px 16px",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><p style={{fontSize:15,fontWeight:700,color:"#F1F5F9",margin:0}}>ปังจัง</p><p style={{fontSize:10,color:"#64748B",margin:0,letterSpacing:"0.5px"}}>LUCKY DRAW ADMIN</p></div>
          </div>
        </div>
        <nav style={{flex:1,padding:"12px 8px",overflowY:"auto",display:"flex",flexDirection:"column",gap:4}}>
          {NAV_GROUPS.map((group,gi)=>(
            <div key={gi} style={{marginBottom:8}}>
              <p style={{fontSize:10,fontWeight:700,color:"#64748B",letterSpacing:"1px",textTransform:"uppercase",padding:"0 10px",margin:"4px 0 6px"}}>{group.label}</p>
              {group.items.map(t=>{
                const isActive=tab===t.key;
                return(<button key={t.key} className="sb-item" onClick={()=>setTab(t.key as any)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:8,border:"none",cursor:"pointer",marginBottom:1,fontSize:13,fontWeight:isActive?600:400,textAlign:"left",transition:"all .15s",background:isActive?"rgba(99,102,241,0.25)":"transparent",color:isActive?"#A5B4FC":"#CBD5E1",borderLeft:isActive?"3px solid #6366F1":"3px solid transparent"}}>
        ...
        <div style={{padding:"12px 8px",borderTop:"1px solid rgba(255,255,255,0.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:"rgba(255,255,255,0.04)",marginBottom:4}}>
            <div style={{width:28,height:28,borderRadius:7,background:"linear-gradient(135deg,#6366F1,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"#fff",flexShrink:0}}>A</div>
            <div style={{flex:1}}><p style={{fontSize:12,fontWeight:600,color:"#F1F5F9",margin:0}}>Administrator</p><p style={{fontSize:10,color:"#64748B",margin:0}}>admin@pangjang.com</p></div>
          </div>
          <button onClick={logout} className="sb-item" style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:8,border:"none",cursor:"pointer",fontSize:12,fontWeight:500,color:"#64748B",background:"transparent"}}>
      ...
      <div style={{flex:1,marginLeft:232,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
        <div style={{background:C.card,borderBottom:`1px solid ${C.border}`,padding:"0 24px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:9,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <span style={{fontSize:12,fontWeight:600,color:C.text}}>{NAV_GROUPS.flatMap(g=>g.items).find(t=>t.key===tab)?.label||tab}</span>
          </div>
          {(pendingCount+pendingMerchantsCount)>0&&(
            <div style={{display:"flex",alignItems:"center",gap:6,background:"#FEF2F2",border:"1px solid rgba(229,62,62,0.2)",borderRadius:8,padding:"4px 10px"}}>
              <div style={{width:6,height:6,borderRadius:"50%",background:C.red}}/>
              <span style={{fontSize:11,fontWeight:600,color:C.red}}>{pendingCount+pendingMerchantsCount} รายการรอ</span>
            </div>
          )}
        </div>
        <div style={{flex:1,padding:24,maxWidth:980,width:"100%"}}>
