...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      `, []),
      // Digital Ticket stats
      safe(async () => ({
        total:     await (prisma as any).digitalTicket.count(),
        unclaimed: await (prisma as any).digitalTicket.count({ where: { status: "unclaimed" } }),
        activated: await (prisma as any).digitalTicket.count({ where: { status: "activated" } }),
        winners:   await (prisma as any).digitalTicket.count({ where: { isWinner: true } }),
      }), { total:0, unclaimed:0, activated:0, winners:0 }),
    ]);
